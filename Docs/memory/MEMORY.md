...
- [SimCopter sound DECODED](simcopter-sound.md) — the 130-slot table (0x00-0x81) + the nine-call API, ported as `USimCopterAudioSubsystem`. Traps: ONE voice per id and Play-while-playing is a NO-OP (that is what stops water droplets machine-gunning); `AddFrequency` is absolute, not cumulative; attenuation is linear to only -40 dB at the 1920-unit (120 m) cull edge, and the cull is an octagon; sirens have no direction at all; CESSLP1 is the plane's engine and DIVE1 is the dive; the rotor loop is per model (COPLOOP2..6) and pitch, not volume, carries it. The old D2### dispatcher mapping was wrong — those ids are DIS053-068. The whole radio tree is zero bytes in this install.
