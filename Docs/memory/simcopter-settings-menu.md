...
New user cities enable Crime at **90** and Riot at **40**, matching the decoded weights.
The remake keeps its starting Difficulty at **0**. Existing saves retain their saved settings.
...
- **New user cities start at Difficulty 0 rather than the original 2.** Mission weights use
  the original defaults, including Crime and Riot.
