...
This yields the original behavior visible in captures: natural terrain blends across water/shore/grass/dirt/rock bands, while roads, buildings, and other man-made surfaces remain crisp. The original water/coastal-water **texture** animation comes from `FUN_004814c0`: `DAT_00503f6c` accumulates `DAT_005039a0`, the game's fixed-time delta, advances above 4000, wraps frame 0..4, and adds that frame to water base type 0 or 5. The former port incorrectly treated 4000 as milliseconds and therefore held each frame for four seconds. Unreal now hard-switches the dedicated terrain-water surface through the five cells at a 4 Hz presentation default; `WATER TEX FPS` in the Ctrl+Alt+D debug panel tunes that playback live (0 freezes frame zero). `M_SimCopterWater` performs the atlas-cell selection in UV space, including base-5's row crossing, and samples only the selected source frame with no adjacent-frame fade. Page 20 (`SKY.BMP` image 4) contains matching water cells, but original mesh pools/ponds using `M_SimCopterCityAtlas` deliberately remain on their authored static cells. Some of those faces are exactly coplanar with terrain or flat pads in the original mesh data; the atlas material therefore raises only the identified mesh-water cells 2 cm in World Position Offset so the underlying face cannot win the depth test, without moving their collision or authored geometry. The water surface's vertical undulation remains a separate effect: when `bAnimateWaterSurface` is set (and a textured terrain surface is in use), water terrain quads are committed to their own `TerrainMeshComponent` section that is drawn with the `M_SimCopterWater` material. Its MID is bound before that procedural section creates its first render state, preventing the plane from appearing late with an initially empty material slot. The material displaces the vertices in the vertex shader (World Position Offset) with a moving sum-of-sines wave and computes analytic per-pixel wave normals, so both animations are GPU-only (no per-frame CPU work or vertex-buffer re-upload). A per-vertex weight baked into vertex-color R pins shoreline verts (0 = touches a land/road/building tile, 1 = open water) so the sea stays welded to the static land surface with no gaps. `M_SimCopterWater` and `M_SimCopterCityAtlas` are authored by `Tools/Unreal/CreateSimCopterMaterials.py`.
