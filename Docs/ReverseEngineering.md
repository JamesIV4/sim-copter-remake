...
- `XZON` high-nibble bits describe SimCity 2000 zone corners, but SimCopter does **not** use them to place building meshes. `FUN_0047c0c0` gets a square size from `FUN_004e4f80`'s `XBLD` table, verifies that the whole square contains the same `XBLD` id, claims those scene cells in row-major order, and places the mesh at the square's center. Treating `XZON & 0x80` as a top-left owner was wrong: in Islandtown it selects the far corner of many 2x2-4x4 buildings and shifts their full GEO meshes onto roads.
...
- Before dispatching a multi-cell object, it validates a 2x2, 3x3, or 4x4 block of identical `XBLD` bytes (except the dedicated 2x2 bridge bands `0x49..0x50` and `0x61..0x6b`). Invalid origins are cleared in the working `XBLD` grid. A successful placement writes one scene-cell pointer into every covered raster cell and stores the center at `origin + (size-1)*0x20` on each horizontal axis. This scene-cell ownership, not `XZON`, suppresses child tiles.
...
This yields the original behavior visible in captures: natural terrain blends across water/shore/grass/dirt/rock bands, while roads, buildings, and other man-made surfaces remain crisp. The original water/coastal-water **texture** animation comes from `FUN_004814c0`: `DAT_00503f6c` accumulates `DAT_005039a0`, the game's fixed-time delta, advances above 4000, wraps frame 0..4, and adds that frame to water base type 0 or 5. The former port incorrectly treated 4000 as milliseconds and therefore held each frame for four seconds. Unreal now plays the five cells at a flowing 4 Hz presentation default; `WATER TEX FPS` in the Ctrl+Alt+D debug panel tunes both terrain and mesh-pool playback live (0 freezes frame zero). `M_SimCopterWater` performs that atlas-cell selection in UV space, including base-5's row crossing, and linearly blends the current and next samples using one shared current/next/blend frame state. This is a presentation divergence from the original hard cell switch: it also blends frame 4 into frame 0, removing the repeating full-surface flash at the wrap without independent frame calculations disagreeing on a boundary render. Page 20 (`SKY.BMP` image 4) contains byte-identical water cells, so `M_SimCopterCityAtlas` applies the same blended cycle to original mesh pools/ponds whose faces use base cell 0 or 5. Some of those faces are exactly coplanar with terrain or flat pads in the original mesh data; the atlas material therefore raises only animated water cells 2 cm in World Position Offset so the underlying face cannot win the depth test, without moving their collision or authored geometry. The water surface's vertical undulation remains a separate effect: when `bAnimateWaterSurface` is set (and a textured terrain surface is in use), water terrain quads are committed to their own `TerrainMeshComponent` section that is drawn with the `M_SimCopterWater` material. Its MID is bound before that procedural section creates its first render state, preventing the plane from appearing late with an initially empty material slot. The material displaces the vertices in the vertex shader (World Position Offset) with a moving sum-of-sines wave and computes analytic per-pixel wave normals, so both animations are GPU-only (no per-frame CPU work or vertex-buffer re-upload). A per-vertex weight baked into vertex-color R pins shoreline verts (0 = touches a land/road/building tile, 1 = open water) so the sea stays welded to the static land surface with no gaps. `M_SimCopterWater` and `M_SimCopterCityAtlas` are authored by `Tools/Unreal/CreateSimCopterMaterials.py`.
