...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FSimCopterGroundIdleTest,
	"SimCopter.Flight.GroundIdle",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

// REMAKE DIVERGENCE: a parked rotor settles to RotorIdleSpeed with the engine running instead of
// winding to a dead stop, so the blades keep turning for as long as the engine loop keeps swishing.
bool FSimCopterGroundIdleTest::RunTest(const FString& Parameters)
{
	const FSimCopterFlightEnvironment Env = GroundAt(0, 0);
	FSimCopterFlightEvents Events;

	FSimCopterFlightInputs Idling;
	Idling.ClimbCommand = 0;
	Idling.bEngineRunning = true;

	FSimCopterFlightInputs ShutDown;
	ShutDown.ClimbCommand = 0;

	// From a running rotor, parked: it decays to idle and stays there.
	FSimCopterFlightModel Model;
	Model.ResetOnSurface(0, 0, 0);
	Model.RotorSpeed = FSimCopterFlightModel::RotorTopSpeed;
	for (int32 Step = 0; Step < 60 * 20; ++Step)
	{
		Model.Step(StepSeconds, Idling, Env, Events);
	}
	TestEqual(
		TEXT("a parked rotor settles exactly on idle and stops there"),
		Model.RotorSpeed,
		FSimCopterFlightModel::RotorIdleSpeed);

	// The whole point: the blades are still moving. Anything else is the dead stop this replaced.
	const int32 AngleBefore = Model.MainRotorAngle;
	Model.Step(StepSeconds, Idling, Env, Events);
	TestNotEqual(TEXT("and the blades are still turning at idle"), Model.MainRotorAngle, AngleBefore);

	// Idle must sit in the band that makes the sound and the picture agree: above the COPLOOP
	// cut-off at 30 so the loop cannot flicker off under it, and below the lift gate so an idling
	// helicopter can never take itself off the ground.
	constexpr int32 RotorSoundGate = 0x1e0000; // 30.0, UpdateHelicopterAudio's gate
	TestTrue(
		TEXT("idle keeps the engine loop alive"),
		FSimCopterFlightModel::RotorIdleSpeed > RotorSoundGate);
	TestTrue(
		TEXT("idle cannot lift"),
		FSimCopterFlightModel::RotorIdleSpeed < FSimCopterFlightModel::RotorLiftGate);
	TestFalse(TEXT("and shows no blur disc"), Model.bRotorBlurDisc);
	TestEqual(TEXT("an idling helicopter stays parked"),
		int32(Model.State), int32(ESimCopterFlightState::Parked));

	// Shutting the engine down winds the same rotor all the way to zero - retail behaviour, which is
	// what the model does whenever bEngineRunning is clear.
	for (int32 Step = 0; Step < 60 * 5; ++Step)
	{
		Model.Step(StepSeconds, ShutDown, Env, Events);
	}
	TestEqual(TEXT("shutting down stops the rotor dead"), Model.RotorSpeed, 0);

	const int32 StoppedAngle = Model.MainRotorAngle;
	Model.Step(StepSeconds, ShutDown, Env, Events);
	TestEqual(TEXT("and the blades stop with it"), Model.MainRotorAngle, StoppedAngle);

	// Holding the collective down through the landing: the shutdown hold elapses while the rotor is
	// still well above idle, so it never settles there - it carries straight on down to a stop. The
	// engine going off must not put a step in the decay either; it only moves the target.
	FSimCopterFlightModel HeldDown;
	HeldDown.ResetOnSurface(0, 0, 0);
	HeldDown.RotorSpeed = FSimCopterFlightModel::RotorTopSpeed;
	// EngineShutdownHoldSeconds is 0.8 s, so the engine stops about 48 steps after touchdown - with
	// the rotor still around 320, nowhere near the idle floor.
	for (int32 Step = 0; Step < 48; ++Step)
	{
		HeldDown.Step(StepSeconds, Idling, Env, Events);
	}
	// Deliberately a loose multiple rather than a tight one: this is asserting "nowhere near idle
	// yet", and it must not start failing every time the idle speed is retuned for feel.
	TestTrue(
		TEXT("the shutdown lands while the rotor is still far above idle"),
		HeldDown.RotorSpeed > FSimCopterFlightModel::RotorIdleSpeed * 2);

	const int32 AtShutdown = HeldDown.RotorSpeed;
	HeldDown.Step(StepSeconds, ShutDown, Env, Events);
	TestTrue(
		TEXT("switching the engine off does not step the rotor, only its target"),
		AtShutdown - HeldDown.RotorSpeed > 0 && AtShutdown - HeldDown.RotorSpeed < 0x40000);

	// Straight through the idle speed without pausing on it, all the way to zero.
	bool bPausedAtIdle = false;
	for (int32 Step = 0; Step < 60 * 10; ++Step)
	{
		const int32 Before = HeldDown.RotorSpeed;
		HeldDown.Step(StepSeconds, ShutDown, Env, Events);
		bPausedAtIdle |= Before == HeldDown.RotorSpeed && Before == FSimCopterFlightModel::RotorIdleSpeed;
	}
	TestFalse(TEXT("a shutting-down rotor never settles on idle"), bPausedAtIdle);
	TestEqual(TEXT("it winds all the way to a stop"), HeldDown.RotorSpeed, 0);

	// A rotor left BELOW idle with the engine on - a save restored on a parked, running helicopter -
	// comes up to it gently rather than snapping, which is why the parked branch converges on a
	// target instead of clamping after the subtraction.
	FSimCopterFlightModel Restored;
	Restored.ResetOnSurface(0, 0, 0);
	Restored.RotorSpeed = 0;
	Restored.Step(StepSeconds, Idling, Env, Events);
	TestTrue(TEXT("a stopped rotor does not snap to idle"),
		Restored.RotorSpeed > 0 && Restored.RotorSpeed < FSimCopterFlightModel::RotorIdleSpeed);
	for (int32 Step = 0; Step < 60 * 5; ++Step)
	{
		Restored.Step(StepSeconds, Idling, Env, Events);
	}
	TestEqual(TEXT("but it does reach it"),
		Restored.RotorSpeed, FSimCopterFlightModel::RotorIdleSpeed);
